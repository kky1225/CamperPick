...
<script type="text/javascript">
	$(function() {
		...
		function selectData(pageNum,market_num) {
			...
			$.ajax({
				type:'post',
				data:{pageNum:pageNum,market_num:market_num},
				url:'listReply.do',
				dataType:'json',
				cache:false,
				timeout:30000,
				success:function(param){
					...
					$(param.list).each(function(index,item) {
						...
						output += item.re_date;
						if($('#mem_num').val() != ''){
							output += ' <input type="button" data-market="'+item.market_num+'" data-num="'+item.mre_num+'" data-mem="'+item.mem_num+'" value="답글 작성" class="write-btn button" style="font-size:14px;"">';
							output += '&nbsp;'
						}
						output +='<input type="button" data-market="'+item.market_num+'" data-num="'+item.mre_num+'" value="답글 보기" class="view-btn button" style="font-size:14px;"">';
						if($('#mem_num').val() == item.mem_num){
							// 로그인 한 회원 번호가 댓글 작성자 회원 번호와 같으면
							output += '	<input type="button" data-market="'+item.market_num+'" data-num="'+item.mre_num+'" data-mem="'+item.mem_num+'" value="수정" class="modify-btn button" style="font-size:14px;">';
							output += '	<input type="button" data-market="'+item.market_num+'" data-num="'+item.mre_num+'" data-mem="'+item.mem_num+'" value="삭제" class="delete-btn button" style="font-size:14px;">';
						}
		...
		$('.paging-button input').click(function() {
			...
			selectData(pageNum,$('#market_num').val());
		});
		...
		$(document).on('click','.modify-btn',function(){
			//댓글 글번호
			var mre_num = $(this).attr('data-num');
			//작성자 회원 번호
			...
			var modifyUI = '<form id="mre_form">';
			   ...
			   modifyUI += '     <input type="submit" value="수정" class="button" style="font-size:14px;">';
			   modifyUI += '     <input type="button" value="취소" class="re-reset button" style="font-size:14px;">';
			   modifyUI += '  </div>';
		...
		$(document).on('click','.delete-btn',function(){
			...
			if(choice){
				$.ajax({
					type:'post',
					url:'deleteReply.do',
					data:{mre_num:mre_num,mem_num:mem_num},
					dataType:'json',
					cache:false,
					timeout:30000,
					success:function(param){
		...
		$(document).on('click','.write-btn',function(){
			...
			var rmodifyUI = '<form id="wre_form">';
			   ...
			   rmodifyUI += '     <input type="submit" value="등록" class="button" style="font-size:14px;">';
			   rmodifyUI += '     <input type="button" value="취소" class="wre-reset button" style="font-size:14px;"">';
			   rmodifyUI += '  </div>';
					...
					success:function(param){
						...
						$(param.list).each(function(index,item){
							...
							if($('#mem_num').val()==item.mem_num){
								...
								output += ' <input type="button" data-market="'+item.market_num+'" data-num="'+item.mrre_num+'" data-mem="'+item.mem_num+'" value="수정" class="rmodify-btn button" style="font-size:14px;"">';
								output += ' <input type="button" data-market="'+item.market_num+'" data-num="'+item.mrre_num+'" data-mem="'+item.mem_num+'" value="삭제" class="rdelete-btn button" style="font-size:14px;"">';
							 }
							output += '</div>';
							output += '</div>';
						});//end of each
		...
		$(document).on('click','.rmodify-btn',function(){
			//대댓글 글번호
			var mrre_num = $(this).attr('data-num');
			//작성자 회원 번호
			var mem_num = $(this).attr('data-mem');
			//대댓글 내용
			...
			var modifyUI = '<form id="remre_form">';
			   modifyUI += '  <input type="hidden" name="mrre_num" id="rcre_num" value="'+mrre_num+'">';
			   modifyUI += '  <input type="hidden" name="mem_num" id="rcmem_num" value="'+mem_num+'">';
			   modifyUI += '  <input type="hidden" name="market_num" id="rcmarket_num" value="'+market_num+'">';
			   modifyUI += '  <textarea rows="3" cols="50" name="re_content" id="rcre_content" class="rep-content">'+re_content+'</textarea>';
			   modifyUI += '  <div id="rcre_first"><span class="letter-count">300/300</span></div>';	
			   modifyUI += '  <div id="rcre_second" class="align-right">';
			   modifyUI += '     <input type="submit" value="수정" class="button" style="font-size:14px;">';
			   modifyUI += '     <input type="button" value="취소" class="rre-reset button" style="font-size:14px;"">';
			   modifyUI += '  </div>';
			   modifyUI += '  <hr size="1" noshade width="90%">';
			   modifyUI += '</form>';
		...
		$(document).on('click','.rdelete-btn',function(){
			...
			$.ajax({
				...
				data:{mrre_num:mrre_num,mem_num:mem_num},
				dataType:'json',
				cache:false,
				timeout:30000,
				success:function(param){
