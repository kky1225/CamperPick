			...
			$.ajax({
				...
				success:function(param){
					...
					$(param.list).each(function(index,item) {
						...
						output += item.re_date;
						if($('#mem_num').val() != ''){
							output += ' <input type="button" data-market="'+item.market_num+'" data-num="'+item.mre_num+'" data-mem="'+item.mem_num+'" value="답글 작성" class="write-btn btn btn-outline-dark button" style="font-size:14px;"">';
							output += '&nbsp;'
						}
						output +='<input type="button" data-market="'+item.market_num+'" data-num="'+item.mre_num+'" value="답글 보기" class="view-btn btn btn-outline-dark" style="font-size:14px;"">';
						if($('#mem_num').val() == item.mem_num){
							// 로그인 한 회원 번호가 댓글 작성자 회원 번호와 같으면
							output += '	<input type="button" data-market="'+item.market_num+'" data-num="'+item.mre_num+'" data-mem="'+item.mem_num+'" value="수정" class="modify-btn btn btn-outline-dark button" style="font-size:14px;">';
							output += '	<input type="button" data-market="'+item.market_num+'" data-num="'+item.mre_num+'" data-mem="'+item.mem_num+'" value="삭제" class="delete-btn btn btn-outline-dark button" style="font-size:14px;">';
						}
					...
					success:function(param){
						...
						$(param.list).each(function(index,item){
							...
							if($('#mem_num').val()==item.mem_num){
								...
								output += ' <input type="button" data-market="'+item.market_num+'" data-num="'+item.mrre_num+'" data-mem="'+item.mem_num+'" value="수정" class="rmodify-btn btn btn-outline-dark button" style="font-size:14px;"">';
								output += ' <input type="button" data-market="'+item.market_num+'" data-num="'+item.mrre_num+'" data-mem="'+item.mem_num+'" value="삭제" class="rdelete-btn btn btn-outline-dark button" style="font-size:14px;"">';
							 }
							output += '</div>';
							output += '</div>';
						});//end of each
